...
|-------------------|--------|
| Тяжёлая форма внутри WordPress, долгий отклик при изменении полей | Лёгкая форма на сайте, расчёт суммы — сразу после изменения данных |
...
```
хануманфест.рф (WordPress + bridge)  ──HTTPS JSON──►  апи.хануманфест.рф (Symfony 7.4)
                                                              │
                    ┌─────────────────────────────────────────┼─────────────────────┐
                    ▼                     ▼                 ▼                     ▼
...
- Хостинг: Timeweb, PHP 8.2, MySQL 8.0, document root `public/`.
